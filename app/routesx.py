...
def getdetails():
    try:
        ...
        #    If it is a tring that means token has expired or is invalid
    ...
    try:
        ...
        if auth_token:
            ...
            if fin_profile is None:
                return jsonify({'status': 'Fail','message': 'profile with this id doesnt exist'}),404
    ...
    try:
        ...
        if auth_token:
            ...
            if Profile.query.filter_by(id=post_data['profile_id']).first() is None:
                return jsonify({'status': 'fail','message': 'profile with profile id doesnt exist'}),404
...
def write_feedback():
    try:
        ...
        if auth_token:
            ...
            if Profile.query.filter_by(id = post_data['profile_id']).first() is None:
                return jsonify({'status': 'fail','message': 'profile with this profile id doesnt exist'}),404
